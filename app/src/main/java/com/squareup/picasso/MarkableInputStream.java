...
final class MarkableInputStream extends InputStream {
  ...
  /** Marks this place in the stream so we can resetAll back to it later. */
   ...
   * {@code resetAll} thru {@code limit}. Since we can't call {@code mark()}
   * without also adjusting the resetAll-to-position on the underlying stream this
  ...
  public void reset(long token) throws IOException {
    if (offset > limit || token < reset) {
      throw new IOException("Cannot resetAll");
    }
    in.reset();
    skip(reset, token);
    offset = token;
