...
/**
 * Created by 13510 on 2015/11/27.
 */
@FragmentFeature(layout = R.layout.fragment_hotel)
public class HotelFragment extends BaseFragment implements OnRecyclerItemClickListener, SwipeRefreshLayout.OnRefreshListener, MasterListView.OnRefreshListener, OnClickListener {
...
//    private TabLayout tabLayout;
    ...
    private boolean isAll = true;//获取全部数据 ， 没有筛选条件

    private boolean priceChecked = false;//是否选了价格排序   tab
    private boolean tableChecked = false;//是否选了桌数排序   tab

    private boolean hasPriceData = false;//有价钱数据
    private boolean hasTableData = false;//有桌数数据

    private int global_min_price = 0;
    private int global_max_price = 0;
    private int global_min_table = 0;
    private int global_max_table = 0;
    ...
    public void mTabTableClick(View view) {
        ...
        if (NetworkUtil.hasConnection(getActivity())) {
            ...
            if (hasPriceData && hasTableData) {//同时选了桌数和价钱
                HttpClient.getInstance().gFiltrateHotelsByAllAndOrderDesc(parentId, pageIndex, pageSize, gDiscount, gGift, global_min_table, global_max_table, global_min_price, global_max_price, "table", tableAsc ? "asc" : "desc", cb);
            } else if (hasPriceData) {//只有价钱 数据
                HttpClient.getInstance().gFiltrateHotelsByPrice(parentId, pageIndex, pageSize, gDiscount, gGift, global_min_price, global_max_price, "table", tableAsc ? "asc" : "desc", cb);
            } else if (hasTableData) {//有桌数数据
                HttpClient.getInstance().gFiltrateHotelsByTableCount(parentId, pageIndex, pageSize, gDiscount, gGift, global_min_table, global_max_table, "table", tableAsc ? "asc" : "desc", cb);
            } else { //只有gift and disccount
    ...
    public void mTabPriceClick(View view) {
        ...
        if (NetworkUtil.hasConnection(getActivity())) {
            ...
            //PRI
            if (hasPriceData && hasTableData) {//同时选了桌数和价钱
                HttpClient.getInstance().gFiltrateHotelsByAllAndOrderDesc(parentId, pageIndex, pageSize, gDiscount, gGift, global_min_table, global_max_table, global_min_price, global_max_price, "price", priceAsc ? "asc" : "desc", cb);
            } else if (hasPriceData) {//只有价钱 数据
                HttpClient.getInstance().gFiltrateHotelsByPrice(parentId, pageIndex, pageSize, gDiscount, gGift, global_min_price, global_max_price, "price", priceAsc ? "asc" : "desc", cb);
            } else if (hasTableData) {//有桌数数据
                HttpClient.getInstance().gFiltrateHotelsByTableCount(parentId, pageIndex, pageSize, gDiscount, gGift, global_min_table, global_max_table, "price", priceAsc ? "asc" : "desc", cb);
            } else { //只有gift and disccount
                HttpClient.getInstance().filtrateHotelsByTableOrPrice(parentId, pageIndex, pageSize, "price", priceAsc ? "asc" : "desc", cb);
            }
    ...
    public void confirmClick(View view) {
        ...
        if ((tableSelect != -1 && tableSelect != 0) && (priceSelect != -1 && priceSelect != 0)) {//桌数/价格筛选
            ...
            switch (priceSelect) {
                case 1:
                    global_min_price = 1;
                    global_max_price = 1999;
                    break;
                case 2:
                    global_min_price = 2000;
                    global_max_price = 3000;
                    break;
                case 3:
                    global_min_price = 3000;
                    global_max_price = 4000;
                    break;
                case 4:
                    global_min_price = 4000;
                    global_max_price = 99999;
                    break;
            }
            ...
            if (NetworkUtil.hasConnection(getActivity())) {
                ...
                if (tableChecked) {
                    ...
                    if (hasTableData && hasPriceData) {
                        ...
                        HttpClient.getInstance().gFiltrateHotelsByAllAndOrderDesc(parentId, pageIndex, pageSize, gDiscount, gGift, 1, 10000, 1, 1000000, "table", tableAsc ? "asc" : "desc", cb);
                    }
                }
            } else {
                try {
//                    dataList = db.findAll(Selector.from(Hotel.class).where("capacityPerTable", "between", new int[]{table_min, table_max}));
                    ...
                    if (currentSelected == 2) {//桌数
                        dataList = db.findAll(Selector.from(Hotel.class).where("capacityPerTable", "between", new int[]{global_min_table, global_max_table})
                                .and("highestConsumption", "between", new int[]{global_min_price, global_max_price}).and("isDiscount", "=", gDiscount).and("isGift", "=", gGift).orderBy("capacityPerTable", tableAsc));
                    } else if (currentSelected == 3) {//价格
                        dataList = db.findAll(Selector.from(Hotel.class).where("capacityPerTable", "between", new int[]{global_min_table, global_max_table})
                                .and("highestConsumption", "between", new int[]{global_min_price, global_max_price}).and("isDiscount", "=", gDiscount).and("isGift", "=", gGift).orderBy("highestConsumption", priceAsc));
                    } else {
                        dataList = db.findAll(Selector.from(Hotel.class).where("capacityPerTable", "between", new int[]{global_min_table, global_max_table})
                                .and("highestConsumption", "between", new int[]{global_min_price, global_max_price}).and("isDiscount", "=", gDiscount).and("isGift", "=", gGift));
                    }
            ...
            if (NetworkUtil.hasConnection(getActivity())) {
                if (tableChecked) {//桌数排序
                    ...
                    //HttpClient.getInstance().gFiltrateHotelsByAllAndOrderDesc(parentId, pageIndex, pageSize, gDiscount, gGift, 0, 0, global_min_price, global_max_price, "table", !tableAsc ? "asc" : "desc", cb);
                } else if (priceChecked) {
                    if (hasTableData && hasPriceData) {
                        HttpClient.getInstance().gFiltrateHotelsByAllAndOrderDesc(parentId, pageIndex, pageSize, gDiscount, gGift, global_min_table, global_max_table, global_min_price, global_max_price, "price", priceAsc ? "asc" : "desc", cb);
                    } else if (hasTableData) {
                        ...
                        HttpClient.getInstance().gFiltrateHotelsByAllAndOrderDesc(parentId, pageIndex, pageSize, gDiscount, gGift, 1, 10000, 1, 1000000, "price", priceAsc ? "asc" : "desc", cb);
                    }
                } else {//全部
                    if (hasTableData && hasPriceData) {
                        HttpClient.getInstance().gFiltrateHotelsByAllAndOrderDesc(parentId, pageIndex, pageSize, gDiscount, gGift, global_min_table, global_max_table, global_min_price, global_max_price, "price", priceAsc ? "asc" : "desc", cb);
                ...
                try {
                    if (currentSelected == 2) {//桌数
                        dataList = db.findAll(Selector.from(Hotel.class).where("capacityPerTable", "between", new int[]{global_min_table, global_max_table}).orderBy("capacityPerTable", tableAsc));
                    } else if (currentSelected == 3) {//价格
                        dataList = db.findAll(Selector.from(Hotel.class).where("capacityPerTable", "between", new int[]{global_min_table, global_max_table}).orderBy("highestConsumption", !priceAsc));
                    } else {
                        dataList = db.findAll(Selector.from(Hotel.class).where("capacityPerTable", "between", new int[]{global_min_table, global_max_table}));
                    }
            ...
            if (NetworkUtil.hasConnection(getActivity())) {
                ...
                try {
                    if (currentSelected == 2) {//桌数
                        dataList = db.findAll(Selector.from(Hotel.class).where("highestConsumption", "between", new int[]{global_min_price, global_max_price}).orderBy("capacityPerTable", tableAsc));
                    } else if (currentSelected == 3) {//价格
                        dataList = db.findAll(Selector.from(Hotel.class).where("highestConsumption", "between", new int[]{global_min_price, global_max_price}).orderBy("highestConsumption", priceAsc));
                    } else {
                        dataList = db.findAll(Selector.from(Hotel.class).where("highestConsumption", "between", new int[]{global_min_price, global_max_price}));
                    }
                    adapter.notifyDataSetChanged();
                } catch (DbException e) {
                    e.printStackTrace();
            ...
            if (NetworkUtil.hasConnection(getActivity())) {
                if (tableChecked) {
                    ...
                    HttpClient.getInstance().gFiltrateHotelsByAllAndOrderDesc(parentId, pageIndex, pageSize, gDiscount, gGift, 0, 0, 0, 0, "table", !tableAsc ? "asc" : "desc", cb);
                } else if (priceChecked) {
                    if (hasTableData && hasPriceData) {
                        HttpClient.getInstance().gFiltrateHotelsByAllAndOrderDesc(parentId, pageIndex, pageSize, gDiscount, gGift, global_min_table, global_max_table, global_min_price, global_max_price, "price", priceAsc ? "asc" : "desc", cb);
                    } else if (hasTableData) {
...
//        }
    ...
    private void getNextPages() {
        ...
        if (tableChecked) {
            if (hasTableData && hasPriceData) {
                HttpClient.getInstance().gFiltrateHotelsByAllAndOrderDesc(parentId, pageIndex, pageSize, gDiscount, gGift, global_min_table, global_max_table, global_min_price, global_max_price, "table", priceAsc ? "asc" : "desc", cb);
            } else if (hasTableData) {
                HttpClient.getInstance().gFiltrateHotelsByTableCount(parentId, pageIndex, pageSize, gDiscount, gGift, global_min_table, global_max_table, "table", priceAsc ? "asc" : "desc", cb);
            } else if (hasPriceData) {
                HttpClient.getInstance().gFiltrateHotelsByPrice(parentId, pageIndex, pageSize, gDiscount, gGift, global_min_price, global_max_price, "table", priceAsc ? "asc" : "desc", cb);
            } else {
                HttpClient.getInstance().gFiltrateHotelsByAllAndOrderDesc(parentId, pageIndex, pageSize, gDiscount, gGift, 1, 10000, 1, 1000000, "table", priceAsc ? "asc" : "desc", cb);
            }
            //HttpClient.getInstance().gFiltrateHotelsByAllAndOrderDesc(parentId, pageIndex, pageSize, gDiscount, gGift, global_min_table, global_max_table, global_min_price, global_max_price, "table", !tableAsc ? "asc" : "desc", cb);
        } else if (priceChecked) {
            if (hasTableData && hasPriceData) {
...
//            HttpClient.getInstance().gFiltrateHotelsByAll(parentId, pageIndex, pageSize, gDiscount, gGift, global_min_table, global_max_table, global_min_price, global_max_price, cb);
        }
...
//        }
    ...
    public void initialize() {
        ...
        mtdialog = new MTDialog.Builder(getActivity())
                ...
                .setOnClickListener(HotelFragment.this)
                .create();
...
//                    refreshLayout.setRefreshing(false);
                    ...
                    try {
                        ...
                        if (hotelList == null || hotelList.size() <= 0) {
                            if (!MTDBUtil.todayChecked(getActivity(), HotelsActivity.TAG)) {
                                MTDBUtil.saveHotel(getActivity(), dataList);
