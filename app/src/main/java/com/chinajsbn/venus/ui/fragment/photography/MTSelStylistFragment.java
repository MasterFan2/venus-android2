...
public class MTSelStylistFragment extends BaseFragment implements OnRecyclerItemClickListener, OnClickListener {

    public static final String TAG_STYLIST_MAJORDOMO = "stylist_majordomo";
    public static final String TAG_STYLIST_SENIOR = "stylist_senior";
...
//    }
    ...
    public void initialize() {
        ...
        mtdialog = new MTDialog.Builder(getActivity())
                ...
                .setOnClickListener(MTSelStylistFragment.this)
                .create();
...
//        recyclerView.addItemDecoration(new HorizontalDividerItemDecoration.Builder(getActivity()).color(getResources().getColor(R.color.gray_400)).build());
        ...
        adapter = new MyAdapter(MTSelStylistFragment.this);
    ...
    public void show() {
        ...
        if (NetworkUtil.hasConnection(getActivity())) {
            dialog.show();
            HttpClient.getInstance().getStylist(1, 1, 20, majordomoCallback);
        } else {
            try {
                List<Photographer> listMajordomo = db.findAll(Selector.from(Photographer.class).where("tag", "=", MTDBUtil.STYLIST_MAJORDOMO));
                List<Photographer> listSenior = db.findAll(Selector.from(Photographer.class).where("tag", "=", MTDBUtil.STYLIST_SENIOR));
            ...
            if (photographers.getCode() == 200) {
                ...
                if (photographers.getData() != null && photographers.getData().size() > 0) {
                    ...
                    if (size == 1) {
                        ...
                        photographer1.setTag(MTDBUtil.STYLIST_MAJORDOMO);
                        finaData.add(new DoublePhotographer("majordomo", photographer1, null));
                        ...
                        photographer2.setTag(MTDBUtil.STYLIST_MAJORDOMO);
                        finaData.add(new DoublePhotographer("majordomo", photographer1, photographer2));
                ...
                try {
                    if (!MTDBUtil.todayChecked(getActivity(), MTSelStylistFragment.TAG_STYLIST_MAJORDOMO)) {

                        db.delete(Photographer.class, WhereBuilder.b("tag", "=", MTDBUtil.STYLIST_MAJORDOMO));
                        Cache cache = new Cache(MTSelStylistFragment.TAG_STYLIST_MAJORDOMO, MTDBUtil.getToday());
                        db.saveOrUpdate(cache);
                        db.saveOrUpdateAll(tempMajordomo);
                    } else {
                        S.o(":::今天已经缓存过[[" + MTSelStylistFragment.TAG_STYLIST_MAJORDOMO);
                    }
                } catch (DbException e) {
                    e.printStackTrace();
                }catch (Exception e){
            ...
            if (photographers.getCode() == 200) {
                ...
                if (size > 0) {
                    try {
                        for (int i = 0; i < size; i++) {
                            Photographer photographer1 = photographers.getData().get(i);
                            photographer1.setTag(MTDBUtil.STYLIST_SENIOR);
                            finaData.add(new DoublePhotographer("senior", photographers.getData().get(i), null));
                        ...
                        if (!MTDBUtil.todayChecked(getActivity(), MTSelStylistFragment.TAG_STYLIST_SENIOR)) {
                            db.delete(Photographer.class, WhereBuilder.b("tag", "=", MTDBUtil.STYLIST_SENIOR));
                            Cache cache = new Cache(MTSelStylistFragment.TAG_STYLIST_SENIOR, MTDBUtil.getToday());
                            db.saveOrUpdate(cache);
    ...
    class MyAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements View.OnClickListener {
        ...
        public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
            ...
            if (holder instanceof MajordomoTagViewHolder) {
                ...
                final int size = photographer.getList().size();
                ...
                if (size == 0) {
                    ...
                    Picasso.with(getActivity()).load(photographer.getList().get(0).getContentUrl() + DimenUtil.getPhotoStylistStringDimen(DimenUtil.screenWidth)).into(seniorHolder.contentImage1);
                    ...
                    Picasso.with(getActivity()).load(photographer.getList().get(0).getContentUrl() + DimenUtil.getPhotoStylistStringDimen(DimenUtil.screenWidth)).into(seniorHolder.contentImage1);
                    Picasso.with(getActivity()).load(photographer.getList().get(1).getContentUrl() + DimenUtil.getPhotoStylistStringDimen(DimenUtil.screenWidth)).into(seniorHolder.contentImage2);
                } else if (size == 3) {
                    seniorHolder.topLayout.setVisibility(View.VISIBLE);
                    ...
                    Picasso.with(getActivity()).load(photographer.getList().get(0).getContentUrl() + DimenUtil.getPhotoStylistStringDimen(DimenUtil.screenWidth)).into(seniorHolder.contentImage1);
                    Picasso.with(getActivity()).load(photographer.getList().get(1).getContentUrl() + DimenUtil.getPhotoStylistStringDimen(DimenUtil.screenWidth)).into(seniorHolder.contentImage2);
                    Picasso.with(getActivity()).load(photographer.getList().get(2).getContentUrl() + DimenUtil.getPhotoStylistStringDimen(DimenUtil.screenWidth)).into(seniorHolder.contentImage3);
                } else if (size == 4) {
                    seniorHolder.topLayout.setVisibility(View.VISIBLE);
                    seniorHolder.bottomLayout.setVisibility(View.VISIBLE);
                    ...
                    Picasso.with(getActivity()).load(photographer.getList().get(1).getContentUrl() + DimenUtil.getPhotoStylistStringDimen(DimenUtil.screenWidth)).into(seniorHolder.contentImage2);
                    Picasso.with(getActivity()).load(photographer.getList().get(2).getContentUrl() + DimenUtil.getPhotoStylistStringDimen(DimenUtil.screenWidth)).into(seniorHolder.contentImage3);
                    Picasso.with(getActivity()).load(photographer.getList().get(3).getContentUrl() + DimenUtil.getPhotoStylistStringDimen(DimenUtil.screenWidth)).into(seniorHolder.contentImage4);
                }
